...
> **Migration source of truth:** `src/main/resources/db/migration/` 및 `src/main/java/db/migration/` (현재 V1~V7)
> **Database:** PostgreSQL
...
장소명과 카카오·네이버 `category_name`에 포함된 키워드로 PlaceType을 보정하는 운영 규칙이다.

| 컬럼 | 타입 | 설명 |
|---|---|---|
| `keyword` | VARCHAR(100) | 장소명·카카오·네이버 카테고리명에서 찾을 키워드. unique |
...
예를 들어 카페 프로필로 수집된 `가정,생활 > 여가시설 > 방탈출카페`는 `방탈출 → ACTIVITY` 규칙으로 재분류한다. 네이버 매칭 후에는 활성 규칙을 먼저 적용하고, 규칙이 없을 때만 네이버 카테고리로 `CAFE`·`MEAL`을 보정한다.
...
| `naver_blog_evidence` | TEXT | 장소명·동네명으로 수집한 블로그 결과 중 AI 태깅에 사용할 최대 10개의 정제된 보조 자료 |
| `naver_blog_status` | VARCHAR | `COLLECTED`, `EMPTY`, `FAILED`. 블로그 보조 수집 결과이며 전체 정제 상태와 분리 |
| `naver_blog_error_message` | VARCHAR(1000) | 블로그 API 호출 실패 원인. `FAILED`일 때만 저장 |
...
| `ai_batch_job_id` | BIGINT | 초기·정기 태깅을 위해 제출한 OpenAI Batch Job. 일반 단건 태깅에는 비어 있음 |
...
### 📋 openai_batch_tagging_jobs

OpenAI Batch API에 제출한 초기·정기 태깅 요청을 보관한다. 사용자 단건 앵커의 즉시 처리에는 사용하지 않는다.

| 컬럼 | 타입 | 설명 |
|---|---|---|
| `openai_batch_id` | VARCHAR(100) | OpenAI가 반환한 비동기 Batch ID |
| `status` | VARCHAR | `SUBMITTING`, `SUBMITTED`, `COMPLETED`, `FAILED` |
| `requested_count` | INTEGER | Batch에 포함한 Temp 수 |
| `error_message` | VARCHAR(1000) | 제출 실패 원인 |
...
| `idx_place_collection_temp_ai_batch_job_id (ai_batch_job_id, id)` | OpenAI Batch 결과를 해당 Temp에 반영 |
...
AI 태깅
→ 사용자 단건 재시도는 일반 Responses API 요청
→ 초기·정기 수집은 JSONL 파일을 OpenAI Batch API에 제출하고 결과 파일을 회수
→ 활성 Tag 코드만 허용하는 동일 Structured Outputs Schema 사용
...
| 버전 | 파일 | 내용 |
|------|------|------|
| V1 | `V1__create_place_and_location_domain.sql` | 도시·Area·태그·Place·Place 통계의 최종 구조와 인덱스 생성 |
| V2 | `V2__create_course_domain.sql` | 코스·일정·상황 적합도·코스 태그·코스 통계의 최종 구조와 인덱스 생성 |
| V3 | `V3__seed_master_data.java` | CSV 기준 도시·태그·태그 선택지·서울 Area와 수집 범위 적재 |
| V4 | `V4__create_place_collection.sql` | 카카오 수집 프로필·분할 Job·장소 분류 규칙 및 초기 수집 프로필 적재 |
| V5 | `V5__create_place_collection_temp.sql` | 카카오 원본, 네이버 장소·블로그 1차 정제 결과, AI 태깅 상태를 보관하는 Temp 테이블 생성 |
| V6 | `V6__create_openai_batch_tagging_jobs.sql` | OpenAI 비동기 Batch Job과 Temp 연결 정보 생성 |
| V7 | `V7__seed_place_collection_data.java` | CSV 기준의 장소 수집 프로필·PlaceType 보정 규칙 적재 |
